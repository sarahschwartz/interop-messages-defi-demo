...
contract InteropToken is ERC20, Ownable {
    ...
    IMessageVerification public l2MessageVerifier =
        IMessageVerification(L2_MESSAGE_VERIFICATION_ADDRESS);
    ...
    constructor(
        ...
        require(
            _chainIds.length == _stakingContracts.length,
            "Length mismatch"
        );
        for (uint256 i = 0; i < _chainIds.length; ++i) {
            require(_stakingContracts[i] != address(0), "Zero address used");
            require(
                approvedStakingContracts[_chainIds[i]] == address(0),
    ...
    function mint(
        ...
        require(
            addressesThatMinted[msg.sender] == false,
            "Token already minted"
        );
        require(
            approvedStakingContracts[_sourceChainId] == _l2MessageData.sender,
            "Message origin not approved"
        );
        bool result = checkMessageProof(
            _sourceChainId,
            _l1BatchNumber,
            _l2MessageIndex,
            _l2MessageData,
